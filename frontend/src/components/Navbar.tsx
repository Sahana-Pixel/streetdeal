...
const handleAuthClick = (mode: "login" | "signup") => {
  ...
  return (
    ...
    >
            ...
            <div className="container mx-auto px-4 py-3 flex flex-col gap-1">
              {navItems.map((item) => (
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  onClick={() => scrollToSection(item.id)}
                  className={`flex items-center gap-3 py-4 px-5 text-lg font-medium rounded-lg transition-all duration-200 ${
                    item.id === "vendor"
                      ? "text-sky-300"
                      : item.id === "supplier"
                      ? "text-indigo-300"
                      : "text-slate-200"
                  } hover:bg-slate-700/50 hover:text-blue-300`}
                >
     ...
     // In Navbar.tsx, update the AuthModal usage:
<AuthModal 
  isOpen={authModalOpen} 
  onClose={() => setAuthModalOpen(false)}
  initialMode="login" // Or pass the mode directly if you have different buttons
/>
    </>
  );
};
